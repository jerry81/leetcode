...
number of buckets, and each bucket can be fed from by any number of pigs. Wait
...
poisonous bucket will die, and all others will survive. Repeat this process
until you run out of time. Given buckets, minutesToDie, and minutesToTest,
return the minimum number of pigs needed to figure out which bucket is poisonous
...
and 3. At time 15, there are 4 possible outcomes:
...
bucket 4. At time 30, one of the two pigs must die, and the poisonous bucket is
the one it was fed.
