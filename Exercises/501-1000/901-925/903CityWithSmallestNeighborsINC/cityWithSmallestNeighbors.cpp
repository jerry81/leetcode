...
Distance Medium Topics Companies Hint There are n cities numbered from 0 to n-1.
Given the array edges where edges[i] = [fromi, toi, weighti] represents a
bidirectional and weighted edge between cities fromi and toi, and given the
...
to return city 3 since it has the greatest number. Example 2:
...
graph. The neighboring cities at a distanceThreshold = 2 for each city are: City
0 -> [City 1] City 1 -> [City 0, City 4] City 2 -> [City 3, City 4] City 3 ->
[City 2, City 4] City 4 -> [City 1, City 2, City 3] The city 0 has 1 neighboring
city at a distanceThreshold = 2.
...
class Solution {
 public:
  int findTheCity(int n, vector<vector<int>>& edges, int distanceThreshold) {
    // edges a,b,dist
    // build nodeid: [neighbor1, neigh2...]
    // shortest paths so bfs
    ...
    for (auto e : edges) {
      ...
      lookup[n1].push_back({n2, dist});
      if (lookup.find(n2) == lookup.end()) {
        lookup[n2] = {};
      }
      lookup[n2].push_back({n1, dist});
    }
