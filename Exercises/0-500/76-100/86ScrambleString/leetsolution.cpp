...
public:
    bool isScramble(string s1, string s2) {
        ...
        int n = (int) s1.size();
        bool vis[n][n][n][n]; // a second 4d array??
        bool dp[n][n][n][n]; // dp is bool, no vector, this is 4d array
        memset(vis, false, sizeof(vis)); // leet memset call
        ...
        function<bool(int, int, int, int)> helper = [&] (int l1, int r1, int l2, int r2) {  // wow what is this!!  a lambda??
            if (vis[l1][r1][l2][r2]) return dp[l1][r1][l2][r2];  // so vis is a visited lookup
            vis[l1][r1][l2][r2] = true;

            bool isSame = true;
            for (int i = l1, j = l2; i <= r1 && j <= r2; i++, j++) // leet for loop usage
