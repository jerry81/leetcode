...
public:
    int  helper(int idx,vector<int>&days,vector<int>&costs,vector<int>&dp){
        ...
        return dp[idx]=min(min(c1,c7),c3); // combine to one line
    }
    int mincostTickets(vector<int>& days, vector<int>& costs) {
        vector<int>dp(days.size()+1,-1); // constructor, init to -1, why the + 1?
        return helper(0,days,costs,dp); // dp is passed in
