...
class Solution {
 public:
  int minGroups(vector<vector<int>>& intervals) {
    sort(intervals.begin(), intervals.end()); // til: vectors sortable

    // type, container type, sorting function
    priority_queue<int, vector<int>, greater<int>> minHeap;

    for (const auto& interval : intervals) {
      // If the current interval starts after the earliest ending interval, pop it
