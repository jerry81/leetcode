...
such that every edge connects a node in one and a node in the other. Example 2:
  ...
  bool bfs(vector<vector<int>> graph, int start) {
    ...
    while (!nextneighbors.empty()) {
      ...
      for (int n : neighbors) {
        ...
        if (teams.find(n) != teams.end() && teams[n] == parentTeam)
          return false;
...
bipartite - partition into 2 independent sets - each edge in graph connects node
in set a and set b
